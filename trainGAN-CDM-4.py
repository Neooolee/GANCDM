...
def main():
    ...
    x_img = tf.placeholder(tf.float32,shape=[args.batch_size, args.image_size, args.image_size,4],name='x_img') #输入的x域图像
    y_img = tf.placeholder(tf.float32,shape=[args.batch_size, args.image_size, args.image_size,4],name='y_img') #输入的y域图像
...
#        k=0
        while (starty+args.batch_size)<=leny:
            ...
            batch_x_image=read_imgs(x_datalists[startx:startx+args.batch_size],65535,k)[:,:,:,1:5]
            batch_y_image=read_imgs(y_datalists[starty:starty+args.batch_size],65535,k)[:,:,:,1:5] #读取x域图像和y域图像            
