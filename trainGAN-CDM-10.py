...
def main():
    ...
    x_img = tf.placeholder(tf.float32,shape=[args.batch_size, args.image_size, args.image_size,10],name='x_img') #输入的x域图像
    y_img = tf.placeholder(tf.float32,shape=[args.batch_size, args.image_size, args.image_size,10],name='y_img') #输入的y域图像
    fake_y,fake_x_,xall,yall,xlogits,ylogits=attention_generator(x_img,y_img)#,z_img)
    xr=xall[:,:,:,0:1]
    xt=xall[:,:,:,1:2]
    yr=yall[:,:,:,0:1]
    ...
    for epoch in range(args.epoch): #训练epoch数       
        ...
        while (starty+args.batch_size)<=leny:
            ...
            batch_x_image=read_imgs(x_datalists[startx:startx+args.batch_size],65535,k)
            batch_y_image=read_imgs(y_datalists[starty:starty+args.batch_size],65535,k) #读取x域图像和y域图像            
            ...
            if step % args.write_pred_every == 0: #每过write_pred_every次写一下训练的可视化结果
                ...
                write_image = get_write_picture([[batch_x_image[0][:,:,[2,3,5]],fake_y_value[0][:,:,[2,3,5]],batch_y_image[0][:,:,[2,3,5]]],[rx[0],ry[0],tx[0]]]) #得到训练的可视化结果
